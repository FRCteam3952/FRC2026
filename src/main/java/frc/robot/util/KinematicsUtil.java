...
public class KinematicsUtil {
    public static Pair<LinearVelocity, Pair<Angle, Angle>> getShooterState(double xPos, double yPos, double xSpeed, double ySpeed) {
        ...
        double hubYaw = Math.atan2(yDiff, xDiff);
        ...
        double mYaw = Math.atan2(mSyS,mSxS);
    ...
    public static Pair<LinearVelocity, Pair<Angle, Angle>> getShooterStateForPass(double xPos, double yPos, double xSpeed, double ySpeed) {
        // System.out.println("xPos: " + xPos + ", yPos: " + yPos + ", xSpeed: " + xSpeed + ", ySpeed: " + ySpeed);
        double g = 9.8;
        double hubRadius = 0.57;
        double ivanConstant = 0.15;
        ...
        Translation2d passLocation = FieldConstants.BLUE_OUTPOST_PASS_TRANSLATION;
        if (DriverStation.getAlliance().isPresent()) {
            if (DriverStation.getAlliance().get() == Alliance.Red) {
                if (yPos > 4.0345) {
                    passLocation = FieldConstants.RED_OUTPOST_PASS_TRANSLATION;
                } else {
                    passLocation = FieldConstants.RED_DEPOT_PASS_TRANSLATION;
                }
            } else {
                if (yPos > 4.0345) {
                    passLocation = FieldConstants.BLUE_DEPOT_PASS_TRANSLATION;
                }
            }
        } else {
            System.out.println("Error: could not get alliance; defaulting to blue");
            if (yPos > 4.0345) {
                passLocation = FieldConstants.BLUE_DEPOT_PASS_TRANSLATION;
            }
        }

        double hubX = passLocation.getX();
        double hubY = passLocation.getY();
    ...
    public static Pair<LinearVelocity, Pair<Angle, Angle>> getShooterStateWithoutHood(double xPos, double yPos, double xSpeed, double ySpeed) {
        ...
        double mSyS = sSyS - ySpeed;
        double mSzS = sSS * Math.sin(sSA);
        double mYaw = /*Angle.ofBaseUnits(*/Math.atan2(mSyS,mSxS);//, Radians).plus(Angle.ofBaseUnits(Math.PI, Radians)).in(Radians);
        double mSS = Math.sqrt(Math.pow(mSxS, 2) + Math.pow(mSyS, 2) + Math.pow(mSzS, 2));
        double mSA = Math.atan(mSzS/Math.sqrt(Math.pow(mSxS, 2) + Math.pow(mSyS, 2)));
